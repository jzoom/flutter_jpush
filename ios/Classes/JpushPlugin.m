...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  ...
  JpushPlugin* instance = [[JpushPlugin allocWithZone:nil] init];
...
} else if ([@"setupWithOption" isEqualToString:method]) {
	NSDictionary* map=call.arguments;
	[self setupWithOption:map[@"launchingOption"] appKey:map[@"appKey"] channel:map[@"channel"] apsForProduction:map[@"isProduction"]];
	result(@YES);
...
+ (id)allocWithZone:(NSZone *)zone {
  static JpushPlugin *sharedInstance = nil;
...
- (void)startup {
  [JPushActionQueue sharedInstance].isReactDidLoad = YES;
...
- (void)networkConnecting:(NSNotification *)notification {
  ...
  [self.channel invokeMethod:@"connectionChange" arguments:@(false)];
}
...
- (void)networkDidSetup:(NSNotification *)notification {
  _isJPushDidLogin = false;
  [self.channel invokeMethod:@"connectionChange" arguments:@(true)];
}
...
//                                               body:@(true)];
  [self.channel invokeMethod:@"networkDidLogin" arguments:nil];
...
- (void)receiveRemoteNotification:(NSNotification *)notification {
  ...
  if ([JPushActionQueue sharedInstance].isReactDidLoad == YES) {
    NSDictionary *obj = [notification object];
